...
import 'package:inventory_tracker/viewmodels/extensions/location_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/room_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/container_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/item_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/move_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/search_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/statistics_provider.dart';
import 'package:inventory_tracker/viewmodels/extensions/utility_provider.dart';
import 'dart:async';

class InventoryProvider extends BaseInventoryProvider
    with
        LocationProvider,
        RoomProvider,
        ContainerProvider,
  ...
  InventoryProvider({InventoryRepository? inventoryRepository})
      : super(inventoryRepository: inventoryRepository) {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      for (final room in roomsList) {
        containersMap.putIfAbsent(room.id, () => []);
        itemsMap.putIfAbsent(room.id, () => []);
        locationNames.add(room.location);
      }

      for (final container in containers) {
        containersMap.putIfAbsent(container.roomId, () => []);
        containersMap[container.roomId]!.add(container);
      }

      for (final item in items) {
        itemsMap.putIfAbsent(item.roomId, () => []);
        itemsMap[item.roomId]!.add(item);
      }
